...
function showPage(pageId) {
    ...
    if(pageId === 'map') setTimeout(() => initKakaoMap(), 100);

    // 아이콘 다시 렌더링 (새로 보여진 요소 때문)
...
function renderDiseaseList() {
    ...
    if (!diseases) {
        listContainer.innerHTML = '<div class="text-center py-10 text-gray-400 col-span-full">데이터 준비 중</div>';
        return;
    }
...
function initKakaoMap() {
    if (typeof kakao === 'undefined') {
        ...
        document.getElementById('map').innerHTML = `
            <div class="w-full h-full bg-gray-200 flex items-center justify-center flex-col text-gray-500">
                <i data-lucide="map-pin" class="w-12 h-12 md:w-16 md:h-16 text-blue-500 mb-2"></i>
                <p class="font-bold text-lg md:text-xl">카카오 지도 API 키를 설정해주세요</p>
                <p class="text-xs md:text-sm mt-2 px-4 text-center">index.html의 YOUR_APP_KEY를 발급받은 키로 교체하세요</p>
            </div>
        `;
        setTimeout(() => lucide.createIcons(), 50);
        return;
